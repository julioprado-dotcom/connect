...
UPDATE FuenteEstado SET horariosOptimos = '[]';

-- 6. Verificar resultados
SELECT 'RESUMEN DE MIGRACIÓN' as info;
SELECT 'Fuentes por estado:' as info;
SELECT estado, COUNT(*) as total FROM FuenteEstado GROUP BY estado;
SELECT 'Frecuencias asignadas:' as info;
SELECT m.nombre, m.categoria, fe.frecuenciaBase, fe.frecuenciaActual, fe.estado
FROM FuenteEstado fe
JOIN Medio m ON fe.medioId = m.id
