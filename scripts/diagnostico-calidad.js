...
function validateInline(contenido, tipo) {
  const rules = {
    EL_TERMOMETRO: { min: 500, max: 850, reqSecc: false, prohibido: ['lo siento','no puedo','como ia','i am','as an ai','N/A','en conclusion','se recomienda'], requerido: ['##'] },
    SALDO_DEL_DIA: { min: 350, max: 550, reqSecc: false, prohibido: ['lo siento','no puedo','como ia','i am','as an ai','N/A','Hits','Miss','en conclusion'], requerido: ['##'] },
    ...
    EL_ESPECIALIZADO: { min: 1200, max: 2200, reqSecc: true, prohibido: ['lo siento','no puedo','como ia','i am','as an ai','se recomienda','es necesario','se debe','en conclusion'], requerido: ['##','hallazgo'] },
    EL_HILO: { min: 500, max: 850, reqSecc: false, prohibido: ['lo siento','no puedo','como ia','i am','as an ai','N/A','en conclusion'], requerido: [] },
  ...
  const totalCitas = citas + notasPie;
  if (totalCitas >= 5) score += 5; else if (totalCitas >= 3) score += 3; else if (totalCitas === 0) score -= 15;
  ...
  return { score, palabras, citas, notasPie, secciones, errores, advertencias };
}

const productos = ['EL_TERMOMETRO','SALDO_DEL_DIA','EL_FOCO','EL_ESPECIALIZADO','EL_HILO','EL_RADAR','BOLETIN_DEL_GRANO'];

console.log('=========================================================');
console.log('  DIAGNOSTICO DETALLADO DE CALIDAD v2');
console.log('=========================================================\n');
...
for (const p of productos) {
  ...
  console.log(status + ' ' + p + ' | DB=' + scoreDB + ' Inline=' + v.score + ' | ' + v.palabras + ' pal | ' + (v.citas+v.notasPie) + ' cit(' + v.citas + 'f+' + v.notasPie + 'np) | ' + v.secciones + ' secc | ' + r.totalMenciones + ' menc | ' + d);
